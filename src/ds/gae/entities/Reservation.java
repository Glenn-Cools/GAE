...
@NamedQueries({
		...
		@NamedQuery(name = "Reservation.FindResWithKey" , query = "SELECT r FROM Reservation r WHERE r.key = :resKey")
})
public class Reservation extends Quote {
	...
	public String toString() {
		return String.format("Reservation for %s from %s to %s at %s\nCar type: %s\tCar: %s\nTotal price: %.2f",
				getCarRenter(), getStartDate(), getEndDate(), getRentalCompany(), getCarType(), car.getId(),
				getRentalPrice());
	}
